...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.bn1(out)
        out = self.relu(out)
        out = self.dropout(out) # removing dropout because apply DP-SGD to the model it is adding enough
                                 # regularization 
...
class ResNet(nn.Module):
    ...
    "The improved ResNet can be decomposed into....
     1) Feature extraction: 1 convolutional layers followed by a batch normalization layer, 
                            a tanh activation function, a max pooling layer, 
